...
export default function SignupClient() {
  const { user, loading, signUp, signInWithGoogle, isMock } = useAuth();
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (err.code === 'auth/email-already-in-use') {
        ...
        message = "Invalid email format. Please correct it.";
      } else if (err.code === 'auth/weak-password') {
        message = "Weak password. Please use a password with at least 6 characters.";
      }
      setErrorMsg(message);
    } finally {
      setIsSubmitting(false);
      ...
      <div className="max-w-md w-full bg-white rounded-3xl border border-[#e2ecec] shadow-sm p-8 md:p-10 space-y-6">
        ...
        <div className="text-center space-y-2">
          ...
          <p className="text-xs text-[#5f7475]">
            Secure, private tracking for your baby's NICU logs.
          </p>
        </div>
          ...
          <div className="p-3.5 rounded-xl bg-teal-50 border border-teal-200 text-teal-900 text-xs flex gap-2 items-start animate-fade-in">
            ...
            <div>
              ...
              <p className="mt-0.5 text-[#5f7475] leading-relaxed">
                Firebase keys are unconfigured. You can register any email/password instantly to test the application locally.
              </p>
            </div>
          </div>
        )}
        ...
        <form onSubmit={handleSignup} className="space-y-4">
          <div>
            <label className="block text-xs font-semibold text-[#5f7475] mb-1.5" htmlFor="signup-email">
              Email Address
            </label>
          ...
          <div>
            <label className="block text-xs font-semibold text-[#5f7475] mb-1.5" htmlFor="signup-password">
          ...
          <div>
            <label className="block text-xs font-semibold text-[#5f7475] mb-1.5" htmlFor="signup-confirm-password">
              Confirm Password
            </label>
            <div className="relative">
              <KeyRound className="absolute left-3.5 top-3 w-4 h-4 text-[#82a596]" />
