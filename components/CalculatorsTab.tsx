...
import { 
  calculateBiliCutoffs, 
  BILI_DATA_BY_GA, 
  PRETERM_BILI_THRESHOLDS 
} from '../lib/bilirubinRules';
...
export default function CalculatorsTab() {
  ...
  const calculatedBiliResult = useMemo(() => {
    return calculateBiliCutoffs(gaWeeks, ageHours, hasNeuroRisk, tsbMgDl);
  }, [gaWeeks, ageHours, hasNeuroRisk, tsbMgDl]);

  const thresholds = calculatedBiliResult;
  const biliStatus = calculatedBiliResult.classification;
  ...
  const pretermCalculatedBiliResult = useMemo(() => {
    // Map pretermGaGroup string to weeks
    let weeks = 32;
    if (pretermGaGroup === '<28') weeks = 27;
    else if (pretermGaGroup === '28-29') weeks = 28;
    else if (pretermGaGroup === '30-31') weeks = 30;
    else if (pretermGaGroup === '32-33') weeks = 32;
    else if (pretermGaGroup === '34') weeks = 34;

    return calculateBiliCutoffs(weeks, 72, pretermHasRisk, pretermTsbMgDl);
  }, [pretermGaGroup, pretermHasRisk, pretermTsbMgDl]);

  const pretermThresholds = pretermCalculatedBiliResult;
  const pretermBiliStatus = pretermCalculatedBiliResult.classification;
